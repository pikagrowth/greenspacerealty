...
export default function ProjectGalleryPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="flex flex-col w-full min-h-screen bg-gray-50 pb-24">
      ...
      {/* Header Area */}
      <div className="bg-white border-b border-gray-200 pt-24 pb-8 sticky top-0 z-30 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <Link 
            href={`/projects/${project.slug}`}
            className="inline-flex items-center text-sm font-semibold text-gray-500 hover:text-brand-primary transition-colors mb-4"
          >
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
              ...
              <div className="flex items-center text-gray-600 text-sm font-medium">
                <MapPin size={16} className="mr-1.5 text-brand-accent shrink-0" />
                {project.location}
              </div>
            </div>
            
            <div className="text-sm font-medium text-gray-500 bg-gray-100 px-4 py-2 rounded-lg inline-block">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12 w-full">
        {project.images.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
            {project.images.map((img, idx) => (
              <a 
                ...
                target="_blank" 
                rel="noreferrer"
                className="group relative block aspect-[4/3] rounded-2xl overflow-hidden bg-gray-200 shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 focus:outline-none focus:ring-4 focus:ring-brand-primary/20"
          ...
          <div className="text-center py-24 bg-white rounded-2xl border border-gray-200">
            ...
            <h3 className="text-lg font-bold text-gray-900 mb-2">No Images Available</h3>
            <p className="text-gray-500">Visuals for this project will be uploaded shortly.</p>
          </div>
        )}
      </div>
