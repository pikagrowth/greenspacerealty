    ...
    <div className="flex flex-col w-full bg-white pb-24">
      ...
      <section className="py-24 bg-gray-900 text-white relative overflow-hidden">
        <div className="absolute inset-0 z-0">
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-b from-gray-900 via-gray-900/80 to-brand-primary"></div>
        </div>
        
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          <Badge variant="brand" className="mb-6 bg-brand-accent text-gray-900 border-none">
            ...
            {locations.map((loc, idx) => (
              <div key={loc.id} id={loc.id} className={`flex flex-col lg:flex-row gap-12 items-center scroll-mt-28 ${idx % 2 !== 0 ? 'lg:flex-row-reverse' : ''}`}>
                ...
                <div className="w-full lg:w-1/2">
                  <div className="relative aspect-[4/3] w-full rounded-3xl overflow-hidden shadow-xl border border-gray-100 group">
                    <img 
                      src={loc.image} 
                      alt={loc.name}
                ...
                <div className="w-full lg:w-1/2">
                  ...
                  <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-6">{loc.name}</h2>
                  ...
                  <div className="space-y-4 mb-10">
                    {loc.highlights.map((highlight, hIdx) => (
                      <div key={hIdx} className="flex items-center gap-4 bg-gray-50 p-4 rounded-xl border border-gray-100">
                        <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center shrink-0 shadow-sm text-brand-primary">
                          {highlight.icon}
                        </div>
      ...
      {/* Infrastructure CTA */}
      <section className="py-20 bg-brand-primary/5 border-t border-brand-primary/10">
...
function Badge({ children, className, variant = "brand" }: { children: React.ReactNode, className?: string, variant?: "brand" | "outline" | "urgent" | "secondary" }) {
  return (
